...
import { setDepartingFlightData } from "./setRoundtripFlightUser";
const DepartingFlightSearch = ({ onDepartingFlightSelected }) => {
  ...
  const handleFlightSelection = (data) => {
    dispatch(setDepartingFlightData(data));
    onDepartingFlightSelected();
  };
        ...
        <div
          ...
          <div className="flex flex-row lg:justify-around w-full items-center h-full ">
            <section className="lg:text-2xl text-xs py-6 lg:py-4 lg:pr-10 font-mono text-center border-r lg:border-r-2 border-light-blue">
              <p>Origin</p>
              <p>{data.slice_data.slice_0.departure.airport.name}</p>
              <p className="text-xs">
                {data.slice_data.slice_0.departure.datetime.date_display}
              </p>
              <p>{data.slice_data.slice_0.departure.datetime.time_12h}</p>
            </section>
            <section className="flex flex-col items-center justify-center">
              <div className="border-b pt-4 mx-2 lg:mx-0 border-navy-blue font-roboto lg:text-2xl text-xs font-bold text-center">
                {data.slice_data.slice_0.airline.name}
              </div>
              <div>
                ...
                <p className="text-center text-xs lg:text-lg">
                  {data.slice_data.slice_0.info.duration}
                </p>
            ...
            <section className="lg:text-2xl text-xs py-6 lg:py-4 lg:pl-10 font-mono text-center border-l lg:border-l-2 border-light-blue">
              <p>Destination</p>
              <p>{data.slice_data.slice_0.arrival.airport.name}</p>
              <p className="text-xs">
                {data.slice_data.slice_0.arrival.datetime.date_display}
              </p>
              <p>{data.slice_data.slice_0.arrival.datetime.time_12h}</p>
            </section>
          </div>
        </div>
      ))}
...
export default DepartingFlightSearch;
